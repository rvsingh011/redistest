...
## IBM Cloud Kubernetes Service ready Helm Charts

This Helm Chart has been configured to be configured and validated on top of IBM Cloud Kubernetes Service and using the IBM Cloud Block Storage plugin. 

> **IMPORTANT**: IBM Cloud Block Storage is persistent, high-performance iSCSI storage that you can add to your apps by using Kubernetes persistent volumes (PVs). Please [follow the installation steps for the IBM Cloud Block Storage plugin installation guide](https://cloud.ibm.com/docs/containers?topic=containers-block_storage#install_block).

Once you installed the plugin, use the following command allows you to download and install all the charts from this repository.
...
$ helm install bitnami-ibm/redis
```

```bash
# Production configuration
$ helm install bitnami-ibm/redis --values values-production.yaml
```
...
```bash
$ helm install --name my-release bitnami-ibm/redis
```
...
This version causes a change in the Redis Master StatefulSet definition, so the command helm upgrade would not work out of the box. As an alternative, one of the following could be done:
  ...
  - Recommended: Create a clone of the Redis Master PVC (for example, using projects like [this one](https://github.com/edseymour/pvc-transfer)). Then launch a fresh release reusing this cloned PVC.
   ...
   ```
   helm install bitnami-ibm/redis --set persistence.existingClaim=<NEW PVC>
   ```
  ...
  - Alternative (not recommended, do at your own risk): `helm delete --purge` does not remove the PVC assigned to the Redis Master StatefulSet. As a consequence, the following commands can be done to upgrade the release
   ...
   helm install bitnami-ibm/redis --name <RELEASE>
   ```
...
```bash
$ helm install --name my-release -f values.yaml bitnami-ibm/redis
```
...
```bash
$ helm install bitnami-ibm/redis --set usePassword=true,usePasswordFile=true,existingSecret=redis-password-file,sentinels.enabled=true,metrics.enabled=true
```
...
```console
$ helm install --name my-release -f ./values-production.yaml bitnami-ibm/redis
```
...
```bash
$ helm install --set persistence.existingClaim=PVC_NAME bitnami-ibm/redis
```
...
### 9.0.0
The metrics exporter has been changed from a separate deployment to a sidecar container, due to the latest changes in the Redis exporter code. Check the [official page](https://github.com/oliver006/redis_exporter/) for more information. The metrics container image was changed from oliver006/redis_exporter to bitnami-ibm/redis-exporter (Bitnami's maintained package of oliver006/redis_exporter).
